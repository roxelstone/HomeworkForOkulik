    ...
    def find_entries(self, log_entries: dict, text: str, unwanted: str, date: str) -> dict:
        if date:
            if date.startswith('..'):
                ...
                log_entries = {key: value for key, value in log_entries.items() if key <= date}
            elif date.endswith('..'):
                date = datetime.strptime(date[:-3], '%Y-%m-%d %H:%M:%S.%f')
                log_entries = {key: value for key, value in log_entries.items() if key >= date}
            elif '/' in date:
                ...
                log_entries = {key: value for key, value in log_entries.items() if date_1 <= key <= date_2}
            else:
                date = datetime.strptime(date, '%Y-%m-%d %H:%M:%S.%f')
                log_entries = {key: value for key, value in log_entries.items() if key == date}

        if text:
            log_entries = {key: value for key, value in log_entries.items() if text.lower() in value.lower()}
        ...
        if unwanted:
            if ',' in unwanted:
                ...
                unwanted = [unwanted]
            for unw in unwanted:
                log_entries = {key: value for key, value in log_entries.items() if unw.strip().lower() not in value.lower()}
